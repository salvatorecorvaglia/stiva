...
- Objects are gzipped on write only when their length is known and at most 8MiB. A compressed object cannot be served from a seekable reader, so every Range request against one had to decompress from byte zero to reach the offset — O(n) per request, on exactly the large text and log files where ranged reads are most useful. Existing objects record whether they were compressed and continue to read back correctly either way.
- The Console download endpoint serves seekable objects through `http.ServeContent`, so Range requests and conditional headers work. It previously always wrote the whole body from the start, which is why seeking in the audio and video previews did nothing.
- Console object listings report the stored `contentType`, and the preview picks its renderer from that rather than from the object key's extension, falling back to the extension when the type is absent or generic.
- The Console API is routed with Go 1.22 `ServeMux` method-and-path patterns instead of a catch-all that hand-parsed the path and dispatched through a fourteen-arm switch. A known path reached with the wrong method now answers 405 and an unknown `/api/` path answers a JSON 404, where previously such a request could fall through to the SPA fallback and be answered with `index.html`.
...
- The Console bucket public-access toggle rejects unrecognised values instead of treating anything other than `"true"` as `false` and returning success — a typo in a call meant to publish a bucket previously reported 200 while doing the opposite.
